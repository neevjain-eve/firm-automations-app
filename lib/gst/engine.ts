...
import { r2, formatINR } from "./money";
...
function matchKeyOf(src: EngineDoc): string {
  return `${src.supplierGstin}|${normalizeInvoiceNumber(src.invoiceNumber)}|${group(src.docType)}`;
}
...
function build(a: {
  ...
  return {
    pairKey: `${b?.id ?? "-"}|${g?.id ?? "-"}`,
    matchKey: matchKeyOf(src),
    status: a.status,
    matchMethod: a.method,
    confidence: a.confidence,
    explanation: a.explanation,
    differences: a.diffs,
    bookInvoiceId: b?.id ?? null,
    gstr2bInvoiceId: g?.id ?? null,
    docType: src.docType,
